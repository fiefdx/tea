...
class PyteaTestCase32(TempTestCase):
    def setUp(self):
        self.EncryptStr = EncryptStr
        self.DecryptStr = DecryptStr
        self.iterations = 32

class PyteaTestCase64(TempTestCase):
    def setUp(self):
        self.EncryptStr = EncryptStr
        self.DecryptStr = DecryptStr
        self.iterations = 64
...
if __name__ == "__main__":
    ...
    print "32 iterations:"
    suite = unittest.TestLoader().loadTestsFromTestCase(PyteaTestCase32)
    unittest.TextTestRunner(verbosity = 2).run(suite)
    print "64 iterations:"
    suite = unittest.TestLoader().loadTestsFromTestCase(PyteaTestCase64)
    unittest.TextTestRunner(verbosity = 2).run(suite)
