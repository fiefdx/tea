...
def str_c_encrypt(v, k, iterations = 32):
    '''
    ...
    for i in xrange(0, len(v), 8):
        if i == 0:
            ...
            strtea.tea_c_encrypt(v_c, k_c, iterations)
...
def str_c_decrypt(v, k, iterations = 32):
    '''
    v is unicode or string
    k is md5 unicode
    iterations must be 32 or 64
    ...
    for i in xrange(0, len(v), 8):
        if i == 0:
            ...
            v0, v1 = struct.unpack(">LL", v[i:i + 8])
            v_c = np.ascontiguousarray([v0, v1], dtype = np.uint32)
            strtea.tea_c_decrypt(v_c, k_c, iterations)
...
def tea_str_c_encrypt(v, k, iterations = 32):
    ...
    strtea.tea_c_str_encrypt(v_c, k_c, v_new_length/4, iterations)
    # strtea.tea_c_str_pointer_encrypt(v_c, k_c, v_new_length/4, iterations)
