...
void tea_encrypt (uint32_t* v, uint32_t* k, uint32_t tea_sum) {
    ...
    for (i=0; i < tea_sum; i++) {                       /* basic cycle start */
        sum += delta;
...
void tea_str_encrypt(uint32_t* v, uint32_t* k, uint32_t length, uint32_t tea_sum) {
    ...
    v_tmp[0] = v[0];
    v_tmp[1] = v[1];
    tea_encrypt(v_tmp, k, tea_sum);
    v[0] = v_tmp[0];
    v[1] = v_tmp[1];
    // printf("(%u, %u)\n", v[0], v[1]);
    // printf("encrypt tea_sum: %d\n", tea_sum);
    cipertext = v[0];
    cipertext <<= 32;
    cipertext |= v[1];
    for (i = 1; i<length; i++){
...
uint32_t tea_str_decrypt(uint32_t* v, uint32_t* k, uint32_t length, uint32_t tea_sum) {
    ...
    v_tmp[0] = v[0];
    v_tmp[1] = v[1];
    tea_decrypt(v_tmp, k, tea_sum);
    v[0] = v_tmp[0];
    v[1] = v_tmp[1];
    // printf("(%u, %u)\n", v[0], v[1]);
    // printf("decrypt tea_sum: %d\n", tea_sum);
    pos = v[0];
    pre_plaintext = v[0];
    pre_plaintext <<= 32;
    pre_plaintext |= v[1];
